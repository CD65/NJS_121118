...
Остановка консольного вывода отправки времени через некоторое время и завершение ответа сервера с возвращением
...
---------------------------------------------------------------------------------------------
1. для устновки зависимостей введите npm install
2. для запуска сервера node server.js
3. интервал тиков и таймаут можно вводить в командной строке следующими способами (в милисекундах):
    а) set INTERVAL=1000 && set STOP=6000 && node server.js
    б) node server.js 1000 6000
    в) можно вообще не вводить и тогда усановятся значения по умолчанию INTERVAL=1000 и STOP=5000
---------------------------------------------------------------------------------------------
